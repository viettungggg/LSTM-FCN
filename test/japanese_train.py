...
def convert_to_nested(X, max_length):
    nested_data = []
...
def train():
    ...
    max_length = max(max([np.stack(row.values).shape[1] for _, row in X_train.iterrows()]),
                     max([np.stack(row.values).shape[1] for _, row in X_test.iterrows()]))
    ...
    X_train_nested = convert_to_nested(X_train_padded, max_length)
    X_test_nested = convert_to_nested(X_test_padded, max_length)
